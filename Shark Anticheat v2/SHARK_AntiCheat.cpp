...
// ── Forward declarations (Log functions defined later in file) ──
void LogAlert(const std::string& tag, const std::string& msg);
void LogOK   (const std::string& tag, const std::string& msg);
...
std::string HttpPost(const std::string& fullUrl, const std::string& payload) {
    ...
    HINTERNET hI = InternetOpenA("SharkAC", INTERNET_OPEN_TYPE_DIRECT, NULL, NULL, 0);
...
std::string CreateDiscordThread() {
    std::string desc =
        "SHARK AntiCheat **started** on this machine.\n"
    ...
    std::string payload =
        ...
        "\"color\":52428,"
        "\"footer\":{\"text\":\"SHARK AntiCheat | " + GetTimestamp() + "\"}}]}";
...
void EnumerateAndTerminateWindowByStyle(DWORD processId) {
    ...
    auto EnumWindowsCallback = [](HWND hwnd, LPARAM lParam) -> BOOL {
        ...
        if (windowProcessId == (DWORD)lParam) {
            ...
            if (style == 0x94000000) {
                std::ostringstream espLog;
                espLog << "ESP overlay window! HWND=0x" << std::hex << (uintptr_t)hwnd;
                LogAlert("ESP", espLog.str());
                ...
                if (PostMessage(hwnd, WM_CLOSE, 0, 0))
                    LogAlert("ESP", "Close request sent to overlay window.");
                else
                    LogWarn("ESP", "Failed to close overlay window.");
            }
        }
        return TRUE;
        };
...
void ScanProcess(DWORD pid)
{
    ...
    if (hProc)
    {
        ...
        if (GetModuleFileNameExW(hProc, nullptr, exePath, _countof(exePath)))
        {
            ...
            std::string exeStr = WstrToStr(exePath);
            if (ok)
                LogOK("SIGNATURE", "EXE signed: " + exeStr);
            else
                LogAlert("SIGNATURE", "EXE UNSIGNED: " + exeStr);
        }
        CloseHandle(hProc);
    }
    ...
    if (hModSnap != INVALID_HANDLE_VALUE)
    {
        ...
        if (Module32FirstW(hModSnap, &me))
        {
            do
            {
                ...
                if (!ok) {
                    LogAlert("DLL", "UNSIGNED module: " + modName + " | " + modPath);
                    std::string msg =
    ...
    if (!GetProcessModuleRanges(pid, ranges)) return;

    LogOK("THREAD", "Scanning threads for injection...");
            ...
            do
            {
                    ...
                    !IsAddressWithinModule(startAddr, ranges))
                {
                    foundSuspicious = true;
                    {
                        ...
                        tlog << "Injected thread! PID=" << pid
                             << " TID=" << te.th32ThreadID
                             << " Addr=0x" << std::hex << startAddr;
                        LogAlert("INJECT", tlog.str());
                    }
    ...
    if (!foundSuspicious)
        LogOK("THREAD", "No injected threads found. PID: " + std::to_string(pid));

    LogOK("SCAN", "Process scan complete. PID: " + std::to_string(pid));
}
...
void MonitorAndRestrictAccess(DWORD targetPid) {
    ...
    while (true) {
        ...
        if (FindProcessByName(L"HD-Player.exe") != targetPid) {
            LogWarn("MONITOR", "HD-Player.exe stopped. Waiting to restart...");
            break;
        }
            ...
            if (DuplicateHandle(hOwningProcess, (HANDLE)handle.Handle, GetCurrentProcess(), &hDuplicate, 0, FALSE, DUPLICATE_SAME_ACCESS)) {
                if (GetProcessId(hDuplicate) == targetPid) {
                    ...
                    else
                    {
                        ...
                        LogAlert("HANDLE", "Suspicious process on HD-Player! PID: " + std::to_string(handle.ProcessId) + " | " + WstrToStr(processPath));
...
void monitorexternal() {
    ...
    while (true) {
        ...
        if (targetPid != 0) {
            ...
            LogOK("MONITOR", "HD-Player.exe found. PID: " + std::to_string(targetPid) + " | Monitoring started.");
            ...
            if (isProcessRunning) {
                LogOK("MONITOR", "Waiting for HD-Player.exe to start...");
...
// ============================================================
//  SHARK CONSOLE UI SYSTEM
//  Animated banner, typewriter, color effects
// ============================================================
...
#define COL_DARK_CYAN  (FOREGROUND_GREEN | FOREGROUND_BLUE)
#define COL_DARK_GREY  (FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE)

void SetCol(WORD col) {
    SetConsoleTextAttribute(g_hCon, col);
}
...
void ClearScreen() {
    ...
    FillConsoleOutputCharacter(g_hCon, ' ', cellCount, homeCoords, &count);
    FillConsoleOutputAttribute(g_hCon, csbi.wAttributes, cellCount, homeCoords, &count);
    SetConsoleCursorPosition(g_hCon, homeCoords);
}
...
void HideCursor(bool hide) {
    ...
    GetConsoleCursorInfo(g_hCon, &ci);
    ci.bVisible = !hide;
    SetConsoleCursorInfo(g_hCon, &ci);
}

// Typewriter effect
void TypeWrite(const std::string& text, WORD col, int delayMs = 18) {
    SetCol(col);
    for (char c : text) {
        std::cout << c;
        std::cout.flush();
...
// Print centered text
void PrintCentered(const std::string& text, WORD col, int width = 62) {
...
void LoadingBar(const std::string& label, int width = 40, int delayMs = 30) {
    SetCol(COL_DARK_CYAN);
    std::cout << "  " << label << "\n  [";
    SetCol(COL_CYAN);
    for (int i = 0; i < width; i++) {
        std::cout << char(219); // block char
        std::cout.flush();
        Sleep(delayMs);
    }
    SetCol(COL_DARK_CYAN);
...
// Single status line
void StatusLine(const std::string& label, const std::string& value, WORD valCol) {
    SetCol(COL_DARK_CYAN);
    std::cout << "  [";
    SetCol(COL_CYAN);
    std::cout << "*";
...
// Horizontal divider
void Divider(char c = '-', int width = 62, WORD col = COL_DARK_CYAN) {
    SetCol(col);
...
void PrintSharkArt() {
    ...
    WORD colors[] = {
        COL_BLUE, COL_BLUE, COL_CYAN,
        COL_CYAN, COL_WHITE_B, COL_CYAN, COL_BLUE
    };
    for (int i = 0; i < 7; i++) {
        SetCol(colors[i]);
        std::cout << "      " << art[i] << "\n";
...
// Flicker effect on a line (for dramatic effect)
void FlickerText(const std::string& text, int times = 4) {
    CONSOLE_SCREEN_BUFFER_INFO csbi;
    GetConsoleScreenBufferInfo(g_hCon, &csbi);
...
void PrintBanner()
{
    ...
    // Console window size বড় করা
    SMALL_RECT windowSize = { 0, 0, 79, 39 };
    SetConsoleWindowInfo(g_hCon, TRUE, &windowSize);
    COORD bufSize = { 80, 40 };
    SetConsoleScreenBufferSize(g_hCon, bufSize);

    ClearScreen();

    // ── Phase 1: SHARK ASCII Art animate ──
    std::cout << "\n\n";
    PrintSharkArt();
    Sleep(200);

    // ── Phase 2: Title flicker ──
    std::cout << "\n";
    std::cout << "   ";
    FlickerText("  >>>  S H A R K   A N T I C H E A T   <<<  ", 5);
    Sleep(100);

    // ── Phase 3: Divider animate ──
    SetCol(COL_CYAN);
    std::cout << "\n  ";
    for (int i = 0; i < 60; i++) {
        std::cout << char(205); // double line
        std::cout.flush();
        Sleep(8);
    }
    std::cout << "\n";

    // ── Phase 4: Info block typewriter ──
    Sleep(100);
    TypeWrite("  DEVELOPER  : Tasfik Abdullah\n", COL_WHITE_B, 12);
    TypeWrite("  VERSION    : 2.0 PROFESSIONAL\n", COL_WHITE_B, 12);
    TypeWrite("  TARGET     : HD-Player.exe\n",    COL_WHITE_B, 12);
    TypeWrite("  MODE       : REAL-TIME PROTECTION\n", COL_WHITE_B, 12);

    // ── Phase 5: Bottom divider ──
    SetCol(COL_CYAN);
    std::cout << "  ";
    for (int i = 0; i < 60; i++) {
        std::cout << char(205);
    ...
    // ── Phase 6: Loading bars ──
    LoadingBar("Initializing Detection Engine  ", 38, 18);
    Sleep(80);
    LoadingBar("Loading Signature Database     ", 38, 18);
    Sleep(80);
    LoadingBar("Starting Guardian System       ", 38, 18);
    Sleep(80);
    LoadingBar("Activating Real-Time Monitor   ", 38, 18);
    Sleep(150);

    // ── Phase 7: Status lines ──
    std::cout << "\n";
    Divider('-', 58, COL_DARK_CYAN);
    StatusLine("THREAD MONITOR  ", "ACTIVE", COL_GREEN_B);
    StatusLine("ESP DETECTION   ", "ACTIVE", COL_GREEN_B);
    StatusLine("HANDLE MONITOR  ", "ACTIVE", COL_GREEN_B);
    StatusLine("MEMORY SCANNER  ", "ACTIVE", COL_GREEN_B);
    StatusLine("AIMBOT DETECTOR ", "ACTIVE", COL_GREEN_B);
    StatusLine("DRIVER SCANNER  ", "ACTIVE", COL_GREEN_B);
    StatusLine("VM DETECTION    ", "ACTIVE", COL_GREEN_B);
    StatusLine("GUARDIAN        ", "ACTIVE", COL_GREEN_B);
    Divider('-', 58, COL_DARK_CYAN);

    // ── Phase 8: Online message ──
    std::cout << "\n";
    std::cout << "  ";
    FlickerText("  [  SHARK ANTICHEAT IS NOW ONLINE  ]  ", 4);
    std::cout << "\n";

    // ── Phase 9: Scan log header ──
    SetCol(COL_DARK_CYAN);
    std::cout << "  ";
    for (int i = 0; i < 60; i++) {
        std::cout << char(196);
...
void LogAlert(const std::string& tag, const std::string& msg) {
    ...
    // timestamp
    time_t now = time(nullptr); struct tm t; localtime_s(&t, &now);
    char ts[16]; snprintf(ts, sizeof(ts), "%02d:%02d:%02d", t.tm_hour, t.tm_min, t.tm_sec);

    SetConsoleTextAttribute(hC, COL_DARK_CYAN);  std::cout << "  [";
    SetConsoleTextAttribute(hC, COL_DARK_GREY);  std::cout << ts;
    SetConsoleTextAttribute(hC, COL_DARK_CYAN);  std::cout << "] [";
    SetConsoleTextAttribute(hC, COL_RED_B);      std::cout << "!";
    SetConsoleTextAttribute(hC, COL_DARK_CYAN);  std::cout << "] [";
    SetConsoleTextAttribute(hC, COL_YELLOW);     std::cout << tag;
    SetConsoleTextAttribute(hC, COL_DARK_CYAN);  std::cout << "] ";
    SetConsoleTextAttribute(hC, COL_WHITE_B);    std::cout << msg << "\n";
    SetConsoleTextAttribute(hC, COL_RESET);
}
...
void LogOK(const std::string& tag, const std::string& msg) {
    ...
    SetConsoleTextAttribute(hC, COL_DARK_GREY);  std::cout << ts;
    SetConsoleTextAttribute(hC, COL_DARK_CYAN);  std::cout << "] [";
    SetConsoleTextAttribute(hC, COL_GREEN_B);    std::cout << "+";
    SetConsoleTextAttribute(hC, COL_DARK_CYAN);  std::cout << "] [";
    SetConsoleTextAttribute(hC, COL_CYAN);       std::cout << tag;
    SetConsoleTextAttribute(hC, COL_DARK_CYAN);  std::cout << "] ";
    SetConsoleTextAttribute(hC, COL_DARK_GREY);  std::cout << msg << "\n";
    SetConsoleTextAttribute(hC, COL_RESET);
}
...
void LogWarn(const std::string& tag, const std::string& msg) {
    ...
    SetConsoleTextAttribute(hC, COL_DARK_GREY);  std::cout << ts;
    SetConsoleTextAttribute(hC, COL_DARK_CYAN);  std::cout << "] [";
    SetConsoleTextAttribute(hC, COL_YELLOW);     std::cout << "*";
    SetConsoleTextAttribute(hC, COL_DARK_CYAN);  std::cout << "] [";
    SetConsoleTextAttribute(hC, COL_YELLOW);     std::cout << tag;
    SetConsoleTextAttribute(hC, COL_DARK_CYAN);  std::cout << "] ";
    SetConsoleTextAttribute(hC, COL_WHITE_B);    std::cout << msg << "\n";
...
void MonitorTime()
{
    ...
    while (g_running.load())
    {
        ...
        snprintf(timeBuf, sizeof(timeBuf),
            "SHARK | Developed By Tasfik Abdullah. %02d:%02d:%02d %s",
        ...
        if (diff > threshold) {
            if (!tampered) {
                ...
                {
                    std::ostringstream driftMsg;
                    driftMsg << "Speed hack / clock manipulation! Drift: "
                             << std::fixed << std::setprecision(3) << sec << "s";
                    LogAlert("TIMEHACK", driftMsg.str());
                }
...
// ── Guardian already running কিনা চেক — Guardian-A এবং Guardian-B আলাদা mutex ──
bool IsGuardianRunning(const std::string& id) {
    std::wstring mutexName = L"Global\\SHARKGuardian" +
        std::wstring(id.begin(), id.end()) + L"Mutex";
...
std::wstring DropGuardianEXEAs(const std::wstring& targetName) {
    ...
    std::wstring srcPath = selfDir + L"\\SHARKGuardian.exe";
...
bool LaunchGuardian(const std::wstring& path, const std::string& id,
    ...
    // Validate inputs
    if (path.empty() || threadId.empty() || webhookUrl.empty()) {
        LogWarn("GUARDIAN", "LaunchGuardian-" + id + ": missing config! tid=" +
            (threadId.empty() ? "EMPTY" : "ok") + " url=" +
            (webhookUrl.empty() ? "EMPTY" : "ok"));
        return false;
    }
    ...
    if (ok) {
        ...
        LogOK("GUARDIAN", "Guardian-" + id + " PID=" + std::to_string(pi.dwProcessId) + " launched OK");
    } else {
        LogWarn("GUARDIAN", "Guardian-" + id + " CreateProcess FAILED! Error=" + std::to_string(GetLastError()));
    }
    return ok;
}
...
void DropAndLaunchGuardian(const std::string& threadId, const std::string& webhookUrl) {
    // Guardian-A
    if (!IsGuardianRunning("A")) {
        std::wstring pathA = DropGuardianEXEAs(L"WinSvcHost32.exe");
        if (!pathA.empty()) {
            if (LaunchGuardian(pathA, "A", threadId, webhookUrl))
                LogOK("GUARDIAN", "Guardian-A launched: WinSvcHost32.exe");
            else
                LogWarn("GUARDIAN", "Guardian-A launch FAILED!");
        } else {
            LogWarn("GUARDIAN", "Guardian-A drop failed! Is SHARKGuardian.exe in the same folder?");
        }
    } else {
        LogOK("GUARDIAN", "Guardian-A already running. Skipping launch.");
    }

    // Guardian-B
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    if (!IsGuardianRunning("B")) {
        std::wstring pathB = DropGuardianEXEAs(L"SysMonHelper64.exe");
        if (!pathB.empty()) {
            if (LaunchGuardian(pathB, "B", threadId, webhookUrl))
                LogOK("GUARDIAN", "Guardian-B launched: SysMonHelper64.exe");
            else
                LogWarn("GUARDIAN", "Guardian-B launch FAILED!");
        } else {
            LogWarn("GUARDIAN", "Guardian-B drop failed! Is SHARKGuardian.exe in the same folder?");
        }
    } else {
        LogOK("GUARDIAN", "Guardian-B already running. Skipping launch.");
    }
}
// ============================================================
    ...
    while (true)
    {
        ...
        for (auto& wt : g_WatchedThreads)
        {
            ...
            if (!GetExitCodeThread(wt.handle, &exitCode) || exitCode != STILL_ACTIVE)
            {
                ...
                LogAlert("WATCHDOG", "Thread KILLED: " + std::string(wt.name) + " TID:" + std::to_string(wt.tid));
            ...
            if (suspendCount > 0)
            {
                ...
                LogAlert("WATCHDOG", "Thread SUSPENDED (count=" + std::to_string(suspendCount) + "): " + std::string(wt.name) + " TID:" + std::to_string(wt.tid) + " >> AUTO-RESUMED");
...
int main()
{
    ...
    // ── Animated Banner (সবার আগে) ──
    PrintBanner();

    LogOK("STARTUP", "Connecting to Discord...");
    g_ThreadId = CreateDiscordThread();
    if (!g_ThreadId.empty())
        LogOK("STARTUP", "Discord thread created. ID: " + g_ThreadId);
    else
        LogWarn("STARTUP", "Discord connection failed. Running offline.");

    LogOK("STARTUP", "Initializing freeze detection...");
    InitFreezeDetection();
    std::thread freezePing(FreezeDetectionPingLoop);
    freezePing.detach();

    LogOK("STARTUP", "Launching Guardian process...");
    if (!g_ThreadId.empty())
        DropAndLaunchGuardian(g_ThreadId, DISCORD_WEBHOOK_URL);

    LogOK("STARTUP", "Starting heartbeat thread...");
    ...
    LogOK("STARTUP", "Launching detection threads...");
    launchThreads();

    LogOK("STARTUP", "Starting internal watchdog...");
    std::thread watchdog(WatchdogLoop);
    watchdog.detach();

    LogOK("STARTUP", "All systems operational. Monitoring HD-Player...");
    LogOK("STARTUP", "SHARK AntiCheat is fully active. Waiting for events.\n");
